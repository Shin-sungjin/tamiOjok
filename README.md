...
# 더미 데이터 삽입 (선택 사항, 개발/테스트용. 테이블은 Flyway가 자동 생성하므로 애플리케이션을 먼저 한 번 띄운 뒤 실행)
psql -h localhost -U postgres -d ecommerce -f db/seed.sql
```

> 스키마는 [Flyway](https://flywaydb.org/)(`src/main/resources/db/migration`)가 관리합니다. 빈 데이터베이스에 애플리케이션을 띄우면 기동 시점에 마이그레이션이 자동 적용됩니다. `spring.jpa.hibernate.ddl-auto=validate`로 설정되어 있어 엔티티와 실제 스키마가 어긋나면 기동 시 바로 에러로 알 수 있습니다. `db/schema.sql`은 `V1__init_schema.sql`과 동일한 내용의 참고용 문서이며, 스키마를 변경할 땐 `db/migration`에 새 버전 마이그레이션 파일(`V2__...`)을 추가하세요. `db/seed.sql`은 테스트 계정과 샘플 데이터가 필요할 때만 실행하세요.
