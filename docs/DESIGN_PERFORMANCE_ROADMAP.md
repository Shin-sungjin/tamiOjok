...
4-1 참고. P1도 이미지 파이프라인(의도적 보류, 아래 참고)을 제외하면 전부 완료 —
다음 세션은 P2부터 시작하거나, 실제 상품 사진이 준비되면 이미지 파이프라인부터
다시 열면 됨.
...
| 이미지 최적화 파이프라인 | ⏸ 보류 (의도적, 2026-07-27 결정) | 코드 확인 결과 애초에 이미지 업로드/저장 기능이 없음 — 관리자가 상품 등록 시 이미지 URL을 직접 타이핑해서 넣는 구조(`ProductImage.imageUrl`은 그냥 문자열 컬럼, 서버가 원본 파일을 전혀 통제하지 않음). `db/seed.sql`에도 `product_images` 삽입이 하나도 없어서, 현재 보이는 이미지는 전부 테스트 중 관리자 화면에 임시로 붙여넣은 것뿐 — 시드 데이터 기준 상품 이미지가 하나도 없는 상태. 방향은 두 가지였음: (a) 실제 업로드+저장(로컬 디스크/S3 등) 기능을 새로 만들거나, (b) 외부 이미지 프록시로 기존 URL을 우회 변환. 사용자에게 확인한 결과 **"지금은 보류, 실제 상품 사진이 생기면 그때 다시 논의"**로 결정 — 저장 방식 자체가 아직 안 정해진 상태에서 "최적화"만 먼저 만드는 건 순서가 안 맞다고 판단. 다음에 이 항목을 다시 꺼낼 때는 이 로그부터 참고.
